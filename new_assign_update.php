...
if (!$error_present) 
{
        ...
        if(!$error)
        {
          ...
          $id_=$_SESSION['update_assign_id'];
    		  $sql="update assignments set title='$assign_title',assignment_ques='$assign_ques',start_date='$start',end_date='$end' where assignment_id='$id_'";
         
          $result=$conn->query($sql);
          if($result)
           {
                header("Location: faculty_assignment.php?course=".$_SESSION['update_assign_course']);
                die();
            }  
              else 
                echo $conn->error;
